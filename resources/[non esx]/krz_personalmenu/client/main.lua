...
function admin_give_money()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('WayZe:GiveCash', amount)
		end
	end
end
...
function admin_give_bank()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('WayZe:GiveBank', amount)
		end
	end
end
...
function AddMenuDiversMenu(menu)
	...
	local ragdollItem = NativeUI.CreateItem(('Tomber | Se Relever'), Description1)
...
function AddStopAnim(menu)
	...
	animStopMenu = _menuPool:AddSubMenu(menu.SubMenu, (' ~r~Stop Animation'))
	
	local stopAnim = NativeUI.CreateItem((' ~r~Stop Animation'), "")
